...
public class UpdatesCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            case "help" -> {
                UpdateMessageUtil.sendHelp(sender, sender.hasPermission("updates.admin"), plugin);
                return true;
            }
            case "clear" -> {
                ...
                ColorUtil.send(sender, plugin.getConfig().getString("messages.cleared"));
                return true;
            }
            case "toggle" -> {
                if (!(sender instanceof Player player)) {
                    ColorUtil.send(sender, plugin.getConfig().getString("messages.player-only"));
                    return true;
                }
                boolean toggled = plugin.getPlayerDataManager().toggleJoinMsg(player.getUniqueId());
                ColorUtil.send(sender, plugin.getConfig().getString(
            ...
            default -> {
                try {
                    ...
                    ColorUtil.send(sender, plugin.getConfig().getString("messages.unknown-subcommand"));
                }
                return true;
            }
        }
